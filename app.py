...
# Procesamos cada archivo cargado y guardamos sus datos en una lista.
strategies_data = []
            ...
            with st.expander("1. Simulación Monte Carlo (Bootstrap Simple)", expanded=True):
                ...
                if st.button("▶️ Ejecutar MC Simple", key=f"btn_simple_{active_name}"):
                    with st.spinner(f"Corriendo MC Simple para {active_name}..."):
                        ...
                        st.subheader("📈 Estadísticas Clave (MC Simple)"); stat_cols = st.columns(len(stats))
                        for idx, (label, value) in enumerate(stats.items()): stat_cols[idx].metric(label, f"${value:,.2f}")
                        fig_hist = go.Figure(go.Histogram(x=final_vals, nbinsx=50)); fig_hist.update_layout(title="Histograma de Capital Final", showlegend=False); st.plotly_chart(fig_hist, use_container_width=True)
            ...
            with st.expander("2. Simulación Monte Carlo (Block Bootstrap)"):
                ...
                if st.button("▶️ Ejecutar MC por Bloques", key=f"btn_bb_{active_name}"):
                    with st.spinner(f"Corriendo MC por Bloques para {active_name}..."):
                        ...
                        if sims_rel_bb is None: st.error("Error: El tamaño de bloque es inválido.")
                        else:
                            ...
                            st.subheader("📈 Estadísticas Clave (Block Bootstrap)"); stat_cols_bb = st.columns(len(stats_bb))
                            for idx, (label, value) in enumerate(stats_bb.items()): stat_cols_bb[idx].metric(label, f"${value:,.2f}")
                            fig_hist_bb = go.Figure(go.Histogram(x=final_vals_bb, nbinsx=50)); fig_hist_bb.update_layout(title="Histograma de Capital Final (Block Bootstrap)", showlegend=False); st.plotly_chart(fig_hist_bb, use_container_width=True)
            ...
            with st.expander("3. Stress Test de Recuperación"):
                ...
                if st.button("▶️ Ejecutar Stress Test", key=f"btn_st_{active_name}"):
                    with st.spinner(f"Corriendo Stress Test para {active_name}..."):
                        ...
                        st.subheader("📊 Estadísticas de Recuperación"); c1, c2 = st.columns(2)
                        c1.metric("% Simulaciones Recuperadas", f"{pct_recov:.1f}%"); c2.metric("Mediana Tiempo Recuperación (ops)", f"{med_ttr:.0f}" if isinstance(med_ttr, (int, float)) else med_ttr)
                        fig_ttr = go.Figure()
